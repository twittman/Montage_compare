...
resizeLR() {
    ...
    FL="${FILENAME%.*}"
        ...
        eightEX=$((firstRes1*8))
        tenEX=$((firstRes1*10))
        twelveEX=$((firstRes1*12))
        fourteenEX=$((firstRes1*14))
        thirtysecondEX=$((firstRes1*32))
        if [[ $secondRes1 = $oneEX ]]; then
            autoScale2="100%"
        elif [[ $secondRes1 = $fourEX ]]; then
            autoScale2="400%"
        elif [[ $secondRes1 = $sixEX ]]; then
            autoScale2="600%"
        elif [[ $secondRes1 = $eightEX ]]; then
            autoScale2="800%"
        elif [[ $secondRes1 = $tenEX ]]; then
            autoScale2="1000%"
        elif [[ $secondRes1 = $twelveEX ]]; then
            autoScale2="1200%"
        elif [[ $secondRes1 = $fourteenEX ]]; then
            autoScale2="1400%"
        elif [[ $secondRes1 = $thirtysecondEX ]]; then
            autoScale2="3200%"
        else
            autoScale2="400%"
        fi
    ${MAGICK_COMMAND} convert ${IMAGE_FIRST} -filter point -resize ${autoScale2} ${TMP_FOLDER}/${IMAGE_FIRST} &&
    ${MAGICK_COMMAND} convert ${IMAGE_SECOND} -filter point -resize ${UPSCALE_FACTOR_SECOND_IMAGE} ${TMP_FOLDER}/${IMAGE_SECOND}
}
...
mont() {
    ...
    ${MAGICK_COMMAND} montage -background black ${TMP_FOLDER}/${IMAGE_FIRST} ${TMP_FOLDER}/${IMAGE_SECOND} -tile ${TILE_DIM} -geometry +0+0 -depth 8 -define png:color-type=2 -depth 8 ${TMP_FOLDER}/${tmpMont} &&
    ...
    RGRh=$((RES3/4)) &&
    ResX4="${RGRw}x${RGRh}" &&
    ${MAGICK_COMMAND} convert -size ${ResX4} xc: -tile _checkerboard6x6.miff -draw "color 0,0 reset" -filter Point -resize ${RES1} -depth 8 ${TMP_FOLDER}/${tmpTiles} &&
